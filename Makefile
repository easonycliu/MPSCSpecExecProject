...
.PHONY: all clean install_deps tools yaml-cpp tfhe oblivious-SEAL osprey mage emp-tool emp-ot emp-agmpc show

all: yaml-cpp tfhe oblivious-SEAL osprey mage emp-tool emp-ot emp-agmpc tools
...
$(DEP_INSTALL_DIR)/tools/%: $(DEP_BUILD_DIR)/tools/%.o $(DEP_INSTALL_DIR)/tools yaml-cpp tfhe osprey oblivious-SEAL mage emp-tool emp-ot emp-agmpc
...
$(DEP_BUILD_DIR)/tools/%.o: $(PROJECT_DIR)/tools/%.cpp $(DEP_BUILD_DIR)/tools yaml-cpp tfhe osprey oblivious-SEAL mage emp-tool emp-ot emp-agmpc
	$(CXX) -std=c++20 -Ofast -DNDEBUG -fPIE -march=native -maes -mrdseed -ggdb3 -pthread \
		...
		-I$(PROJECT_DIR)/install/emp-agmpc/include \
		-c $< -o $@
...
emp-agmpc: $(DEP_INSTALL_DIR)/emp-agmpc $(DEP_INSTALL_DIR)/emp-tool $(DEP_INSTALL_DIR)/emp-ot
	cd $(PROJECT_DIR)/emp-agmpc && \
		cmake -B $(DEP_BUILD_DIR)/emp-agmpc \
		...
		cmake --build $(DEP_BUILD_DIR)/emp-agmpc --verbose -j$(JOBS) && \
		cmake --install $(DEP_BUILD_DIR)/emp-agmpc

$(DEP_INSTALL_DIR)/emp-agmpc: $(DEP_INSTALL_DIR) $(DEP_BUILD_DIR)/emp-agmpc
	mkdir $@ || true

$(DEP_BUILD_DIR)/emp-agmpc: $(DEP_BUILD_DIR)
	mkdir $@ || true
