...
if [ "${calculate_only}" != "true" ]; then
	$CKKS_UTILS keygen
	$EXAMPLE_INPUT real_sum ${input_size} 1
	$CKKS_UTILS encrypt_file ${batch_size} 2 real_sum_${input_size}_0_garbler.input
	echo size is ${input_size}
fi

echo expected output is $(od -An -f real_sum_${input_size}_0.expected)
if [ "${tool}" == "osprey" -o "${tool}" == "baseline" ]; then
	...
	$SUDO ${tool_cmd} \
		$CKKS_UTILS real_sum $(( input_size / batch_size )) $round_num real_sum_${input_size}_0_garbler.input real_sum_${input_size}_0_garbler.output | tee -a ${log_file}
...
parties:
    ...
    - workers:
        - internal_host: localhost
          ...
          round_num: ${round_num}
EOF
	$SUDO $PLANNER real_sum ckks config.yaml 0 0 ${input_size} | tee -a ${log_file}
	$SUDO ${tool_cmd} ckks config.yaml 0 0 real_sum_${input_size} | tee -a ${log_file}
	$SUDO mv real_sum_${input_size}_0.output real_sum_${input_size}_0_garbler.output
fi
$CKKS_UTILS decrypt_file 1 real_sum_${input_size}_0_garbler.output
echo real output is $(od -An -f real_sum_${input_size}_0_garbler.output)
