...
if [ "${enable_log}" == "true" ]; then
	...
	log_file=${log_dir}/real_tiled_matrix_multiply.log
	mkdir -p ${log_dir}
	touch ${log_file}
...
if [ "${calculate_only}" != "true" ]; then
	$CKKS_UTILS keygen
	$EXAMPLE_INPUT real_tiled_matrix_multiply ${input_size} 1 random
	$CKKS_UTILS encrypt_file ${batch_size} 1 real_tiled_matrix_multiply_${input_size}_0_garbler.input
	echo size is ${input_size}
fi

echo expected output is $(od -An -f real_tiled_matrix_multiply_${input_size}_0.expected | tail -n 2)
if [ "${tool}" == "osprey" -o "${tool}" == "baseline" ]; then
	...
	$SUDO ${tool_cmd} \
		$CKKS_UTILS real_tiled_matrix_multiply $(( input_size / batch_size )) 16 real_tiled_matrix_multiply_${input_size}_0_garbler.input real_tiled_matrix_multiply_${input_size}_0_garbler.output 2>&1 | tee -a ${log_file}
...
EOF
	$SUDO $PLANNER real_tiled_matrix_multiply ckks config.yaml 0 0 ${input_size} | tee -a ${log_file}
	$SUDO ${tool_cmd} ckks config.yaml 0 0 real_tiled_matrix_multiply_${input_size} | tee -a ${log_file}
	$SUDO mv real_tiled_matrix_multiply_${input_size}_0.output real_tiled_matrix_multiply_${input_size}_0_garbler.output
fi
$CKKS_UTILS decrypt_file 1 real_tiled_matrix_multiply_${input_size}_0_garbler.output
echo real output is $(od -An -f real_tiled_matrix_multiply_${input_size}_0_garbler.output | tail -n 2)
