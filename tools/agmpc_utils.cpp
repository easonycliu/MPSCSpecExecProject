...
CMPC<nP>* setup(int party, int port, std::string circuit_file, NetIOMP<nP>** io, NetIOMP<nP>** io2, ThreadPool** pool,
				BristolFormat** cf) {
	*io = new NetIOMP<nP>(party, port);
	*io2 = new NetIOMP<nP>(party, port + 2 * (nP + 1) * (nP + 1) + 1);

	NetIOMP<nP>* ios[2] = {*io, *io2};

	*pool = new ThreadPool(2 * (nP - 1) + 2);
	*cf = new BristolFormat(circuit_file.c_str());
...
int main(int argc, char** argv) {
	if (argc != 6) {
		std::cout << "Usage: " << argv[0] << " <party> <port> <problem_name> <circuit_dir> <output_path>" << std::endl;
		return 0;
	}
	...
	std::string circuit_dir = argv[4];
	std::string output_path = argv[5];

	NetIOMP<nP>*io, *io2;
	ThreadPool* pool;
	BristolFormat* cf;
	CMPC<nP>* mpc = setup<nP>(party, port, circuit_dir + "/" + problem_name + ".txt", &io, &io2, &pool, &cf);
