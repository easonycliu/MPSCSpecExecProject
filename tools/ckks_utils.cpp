...
int main(int argc, char** argv) {
    ...
    if (std::strcmp(argv[1], "keygen") == 0) {
        ...
        std::cout << "Slot count is " << encoder.slot_count() << std::endl;
        {
            ...
            encoder.encode(1 / static_cast<double>(problem_size), context_data->parms_id(), ckks_scale,
                           reciprocal_size);
        ...
        for (int row_a = 0; row_a != problem_size; row_a++) {
            for (int col_b = 0; col_b != problem_size; col_b++) {
                evaluator.multiply(input_points_a[row_a * problem_size], input_points_b[col_b * problem_size],
                                   result[row_a * problem_size + col_b]);
                for (int i = 1; i != problem_size; i++) {
                    seal::Ciphertext temp;
                    evaluator.multiply(input_points_a[row_a * problem_size + i],
                                       input_points_b[col_b * problem_size + i], temp);
        ...
        for (int row_matrix = 0; row_matrix != problem_size; row_matrix++) {
            evaluator.multiply(input_points_matrix[row_matrix * problem_size], input_points_vector[0],
                               result[row_matrix]);
            for (int i = 1; i != problem_size; i++) {
                seal::Ciphertext temp;
                ...
                for (std::size_t batch_cols_a_rows_b = 0; batch_cols_a_rows_b < problem_size;
                    ...
                    for (std::size_t row_a = batch_row_a; row_a < problem_size && row_a < batch_row_a + tile_size;
                        ...
                        for (std::size_t col_b = batch_col_b; col_b < problem_size && col_b < batch_col_b + tile_size;
                            ...
                            evaluator.multiply(input_points_a[row_a * problem_size + batch_cols_a_rows_b],
                                               input_points_b[col_b * problem_size + batch_cols_a_rows_b],
                                               dot_product_result);
                            for (std::size_t i = 1; i < dot_product_size; i++) {
                                seal::Ciphertext temp;
                                evaluator.multiply(input_points_a[row_a * problem_size + batch_cols_a_rows_b + i],
                                                   input_points_b[col_b * problem_size + batch_cols_a_rows_b + i],
                                                   temp);
