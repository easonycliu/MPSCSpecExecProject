...
void write_record(
	mage::util::BinaryFileWriter* to, std::uint32_t key, std::uint32_t data1 = 0, std::uint32_t data2 = 0,
...
int main(int argc, char** argv) {
	...
	if (problem_name == "aspirin" || problem_name == "aspirin_seq") {
		...
		if (option != "check") {
			...
			for (int w = 0; w != num_workers; w++) {
				std::string output_file_name(
					problem_name + "_" + std::to_string(input_size) + "_" + std::to_string(w) + ".output"
				);
		...
		for (std::uint64_t i = 0; i != input_size; i++) {
			...
			garbler_writers[blocked_party]->write32(0);
			garbler_writers[blocked_party]->write32(0);
		}

		for (std::uint64_t i = 0; i != input_size; i++) {
			std::uint64_t blocked_party = get_blocked_worker(i, num_workers, input_size);
			evaluator_writers[blocked_party]->write32(0);
		}
