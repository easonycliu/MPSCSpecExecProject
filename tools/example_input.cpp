...
void write_record(mage::util::BinaryFileWriter* to, std::uint32_t key, std::uint32_t data1 = 0, std::uint32_t data2 = 0,
...
void write256(mage::util::BinaryFileWriter* to, std::uint64_t value0, std::uint64_t value1, std::uint64_t value2,
              std::uint64_t value3) {
    to->write64(value0);
    to->write64(value1);
    to->write64(value2);
...
int main(int argc, char** argv) {
    ...
    if (option != "check") {
        for (int i = 0; i != num_workers; i++) {
            ...
            garbler_writers[i] =
                std::make_unique<mage::util::BinaryFileWriter>((common_prefix + "_garbler.input").c_str());
            evaluator_writers[i] =
                std::make_unique<mage::util::BinaryFileWriter>((common_prefix + "_evaluator.input").c_str());
    ...
    if (problem_name == "aspirin" || problem_name == "aspirin_seq") {
        ...
        if (option != "check") {
            ...
            for (int w = 0; w != num_workers; w++) {
                std::string output_file_name(problem_name + "_" + std::to_string(input_size) + "_" + std::to_string(w) +
                                             ".output");
                mage::util::BinaryFileReader r(output_file_name.c_str());
                if (r.get_file_length() != expected_bytes_per_worker) {
                    std::cerr << "Expected " << expected_bytes_per_worker << " bytes in " << output_file_name
                              << ", but found " << r.get_file_length() << " bytes" << std::endl;
                    fail = true;
                    continue;
                }
                ...
                if (actual.size() == expected.size()) {
                    ...
                    for (std::uint64_t i = 0; i != actual.size(); i++) {
                        if (actual[i] != expected[i]) {
                            std::cerr << "Actual and expected differ at position " << i << " in the sorted result"
                                      << std::endl;
                            fail = true;
                        }
                    }
                } else {
                    std::cerr << "Expected " << expected.size() << " items in the join, but only found "
                              << actual.size() << " items" << std::endl;
                    fail = true;
                }
            }
            if (fail) {
            ...
            for (std::uint64_t i = 0; i != input_size; i++) {
                for (std::uint64_t j = 0; j != input_size; j++) {
                    ...
                    garbler_writers[get_blocked_worker(i * input_size + j, num_workers, input_size * input_size)]
                        ->write8(elem);
                    evaluator_writers[get_blocked_worker(i * input_size + j, num_workers, input_size * input_size)]
                        ->write8(elem);
        ...
        if (option == "") {
            ...
            std::cerr << "Unknown option " << option << std::endl;
               ...
               problem_name == "real_tiled_16_matrix_multiply" || problem_name == "real_tiled_64_matrix_multiply") {
        /* Layout of A is row-major, blocked. */
        /* Layout of B is column-major, blocked. */
        /* Result matrix is 2D-blocked. */
        assert(mage::util::is_power_of_two(num_workers));
