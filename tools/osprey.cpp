...
void launch_speculative_process(SpeculativeProcessInfo& speculative_info, OspreyConfig& config,
                                const osprey::util::Overlay* speculative_overlay, int speculative_memprog_fd) {
...
void programmed_backing_thread_function(const OspreyConfig& config, const ProgrammedProcessInfo& programmed_info) {
     ...
     */
    osprey::memprog::Buffered3POProgrammer buffered_3po_programmer(programmed_info.child_pid, programmed_info.trace_fd,
                                                                   config.mem_limit_low, config.mem_limit_high,
...
bool parse_osprey_args(OspreyConfig& config, int osprey_argc, char** osprey_argv) {
    ...
    std::transform(config.tracing_algorithm.begin(), config.tracing_algorithm.end(), config.tracing_algorithm.begin(),
                   [](unsigned char c) {
                       return std::toupper(c);
                   });
    std::transform(config.programming_algorithm.begin(), config.programming_algorithm.end(),
                   config.programming_algorithm.begin(), [](unsigned char c) {
                       return std::toupper(c);
                   });
