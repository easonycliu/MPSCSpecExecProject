...
int main(int argc, char** argv) {
	if (argc != 7) {
		std::cerr << "Usage: " << argv[0]
				  << " program_name protocol/plugin config.yaml party_id worker_index input_size" << std::endl;
	...
	if (prot == nullptr) {
		...
		if (plug == nullptr) {
			std::cerr << protocol
					  << " is not a valid protocol name or plugin name. "; // lack of std::endl is intentional
	...
	std::cout << "Phase Times (ms): " << stats.placement_duration.count() << " " << stats.replacement_duration.count()
			  << " " << stats.scheduling_duration.count() << std::endl;
